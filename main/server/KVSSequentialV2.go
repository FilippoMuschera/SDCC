...
func (kvs *KVSSequential) WaitUntilExecutable(request utils.Args) {
	...
	*/
	...
	<-cond0 //Aspetta la condizione 0. Proseguo solo quando il messaggio è effettivamente quello atteso

	fmt.Println("cond0 is now ok")
	//Ora è necessario creare un messaggio associato a questa richiesta.
	kvs.logicalClock.clockMutex.Lock()
	kvs.logicalClock.clockValue += 1
	msg := utils.NewMessage(request, kvs.logicalClock.clockValue, 0)
	...
	go func() {
		for {
			if kvs.checkIfFirstInQueue(request) {
				cond1 <- true
				return
			}
			time.Sleep(SLEEP_TIME)
	...
	go func() {
		for {
			if kvs.checkForAllAcks(request) {
				cond2 <- true
				return
			}
			time.Sleep(SLEEP_TIME)
	...
	go func() {
		for {
			if kvs.checkForNextMsgFormServers(request) {
				cond3 <- true
				return
			}
			time.Sleep(SLEEP_TIME)
